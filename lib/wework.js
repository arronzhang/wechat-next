...
class Wework {
  constructor(params = {}) {
    ...
    this.$req = axios.create({
      ...
      timeout: 1000 * 10,
      responseType: 'json',
      transformRequest: axios.defaults.transformRequest.concat([util.mediaFormData]),
      transformResponse: axios.defaults.transformResponse.concat([util.apiResponseError]),
      method: 'get'
    })
  }
  ...
  request(path, config = {}) {
    ...
    return this.$req.request(path, config).then(res => res.data)
  }
  ...
  post(path, data, config) {
    return this.authorizeRequest(path, Object.assign({ method: 'post', data }, config))
  }
...
Wework.define('getUserInfo', 'get', 'user/getuserinfo', ['code'])
