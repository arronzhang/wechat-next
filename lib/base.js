...
class Base {
  ...
  /**
   * Request wx work api
   * It will automatic add access token to params and throw wx work error message.
   *
   * @api public
   * @param {String} path
   * @param {Object} config
   ...
   * Call the `getAccessToken(defaultParams)` and `saveAccessToken(tokenData, defaultParams)` api if has set by `new WxWork({getAccessToken, saveAccessToken})`.
   *
   */
