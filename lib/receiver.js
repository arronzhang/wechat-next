...
const Receiver = (module.exports = {
  ...
  express(defaultConfig, handler) {
    return function WechatReceiver(req, res, next) {
  ...
  handleRequest(config, method, query, xml, handler) {
    ...
    if (false === msg) {
      ...
      return Promise.resolve(handler(msg.message, msg.appid || config.appid)).then(replyMessage => {
        return {
          status: 200,
  ...
  parse(config, query, data) {
    ...
    if (signature === crypto.shasum(...check)) {
      if (encrypted) {
        ...
        data = data && data.xml
        if (!data) return false
        return { message: data, appid: ret.id }
      }
      return { message: data }
    }
    return false
  ...
  stringify(config, query, receiveMessage, replyMessage) {
    ...
    if (encrypted) {
      let encryptedXML = crypto.encrypt(config.aes_key, config.appid, xml)
